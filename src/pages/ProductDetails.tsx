...
const ProductDetails = () => {
    ...
    const getProducts = (): Promise<IProduct[]> =>
        axios.get(`products/${id}`).then((response) => response.data)

    const { data: product } = useQuery({ queryKey: ['products', id], queryFn: getProducts })
    ...
    return (
        <section className="mt-[82px]">
            <div className="grid grid-cols-2 mb-[180px] gap-10  mx-[50px]">
                <div className="">
                    <img src={(product as unknown as IProduct)?.image} alt="" className="h-[618px] object-cover" />
                </div>
                <div className="">
                    <Text variant="heading-one">{(product as unknown as IProduct)?.name}</Text>
                    <Text className="my-7" variant="subheading-one">{(product as unknown as IProduct)?.price}</Text>
                    <Text variant="body-two">{(product as unknown as IProduct)?.description}</Text>
                    <Button size="small" className="mt-14" onClick={() => {
                        if (!product) return
                        const cartItem: RawCartItem = {
                            image: (product as unknown as IProduct)?.image,
                            name: (product as unknown as IProduct)?.name,
                            price: (product as unknown as IProduct)?.price,
                            product: (product as unknown as IProduct)?._id
                        }
                        addItemToCart({
                            ...cartItem,
                        })
